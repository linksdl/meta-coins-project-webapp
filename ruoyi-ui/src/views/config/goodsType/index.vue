<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="类型名称" prop="goodsTypeName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          @click="handleAdd"
          v-hasPermi="['config:goodsType:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['config:goodsType:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['config:goodsType:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['config:goodsType:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="goodsTypeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column sortable label="名称" align="center" prop="goodsTypeName" :show-overflow-tooltip="true" />
      <el-table-column sortable label="描述" align="center" prop="goodsTypeDesc" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="626px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="68px">
        ...
        <el-form-item label="名称" prop="goodsTypeName">
          <el-input v-model="form.goodsTypeName" placeholder="请输入类型名称" />
        </el-form-item>


        <el-form-item label="描述" prop="goodsTypeDesc">
          <el-input v-model="form.goodsTypeDesc" placeholder="请输入描述" />
        </el-form-item>
...
<script>
import { listGoodsType, getGoodsType, delGoodsType, addGoodsType, updateGoodsType } from "@/api/config/goodsType";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const goodsTypeId = row.goodsTypeId || this.ids
      getGoodsType(goodsTypeId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const goodsTypeIds = row.goodsTypeId || this.ids;
      this.$modal.confirm('是否确认删除商品类型编号为"' + goodsTypeIds + '"的数据项？').then(function() {
        return delGoodsType(goodsTypeIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('config/goodsType/export', {
        ...this.queryParams
