<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :row-class-name="tableRowClassName" :data="transferList" @selection-change="handleSelectionChange">
      ...
      <el-table-column sortable label="名称" align="center" prop="transferName" :show-overflow-tooltip="true" />
      <el-table-column sortable label="类型" align="center" prop="transferType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.config_function_transfer" :value="scope.row.transferType"/>
        </template>
      </el-table-column>
      <el-table-column sortable label="金额" align="center" prop="transferAmount" :show-overflow-tooltip="true" />
      <el-table-column sortable label="转账日期" align="center" prop="transferDatetime" width="180">
        <template slot-scope="scope">
          <span>{{parseTime(scope.row.transferDatetime, '{y}-{m}-{d} {h}:{i}:{s}')}}</span>
        </template>
      </el-table-column>
      <el-table-column sortable label="凭证" align="center" prop="transferImgs" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.transferImgs" :width="25" :height="25"/>
        </template>
      </el-table-column>
      <el-table-column sortable label="描述" align="center" prop="transferDesc" :show-overflow-tooltip="true" />
      <el-table-column sortable label="备注" align="center" prop="remark" :show-overflow-tooltip="true" />
      <el-table-column sortable label="标签" align="center" prop="transferLabelName" :show-overflow-tooltip="true" />
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="金额" prop="transferAmount">
                <el-input-number v-model="form.transferAmount" type="input-number" :min="0.01" :step="0.01" :precision="2" :max="999999999.00" placeholder="请输入内容"/>
              </el-form-item>
            </el-col>
          ...
          <el-row>
           ...
           <el-col :span="8" v-if='false'>
              <el-form-item label="项目" clearable filterable prop="transferProjectId">
                <el-select clearable v-model="form.transferProjectId" filterable placeholder="请选择项目名称">
                  <el-option
                    clearable
      ...
      <el-row>
        <el-col :span="24">
           <el-form-item label="描述" prop="transferDesc">
            <el-input :disabled="true" v-model="form.transferDesc" type="textarea" clearable placeholder="请输入内容" />
          </el-form-item>
        </el-col>
      </el-row>
  ...
  tableRowClassName({row, rowIndex}) {
      ...
      handleTransferTypeChange(val) {
        ...
        getMoneyOptionSelect(this.queryParams).then(response => {
          ...
          {
              this.form.transferMoneyId = this.moneyOptions[0].moneyId;
          }
        });
        ...
        getMemberOptionSelect(this.queryParams).then(response => {
          ...
          {
            this.form.transferMemberId = this.memberOptions[0].memberId;
          }
        });
        ...
        getWeatherOptionSelect(this.queryParams).then(response => {
          ...
          {
            this.form.transferWeatherId = this.weatherOptions[0].weatherId;
          }
        });
        ...
        getEmotionOptionSelect(this.queryParams).then(response => {
          ...
          {
            this.form.transferEmotionId = this.emotionOptions[0].emotionId;
          }
        });

        this.form.transferCityId = this.cityOptions[0].cityId;
    },
    handleSelectProjectName(val) {
      this.projectOptions.forEach((item, index)=>{
        if(item.projectId === val)
        {
          this.form.transferProjectName = item.projectName;
          this.form.transferProjectId = item.projectId;
        }else {
          this.form.transferProjectId = null;
        }
      });
    },
    /** 查询账户下拉 */
